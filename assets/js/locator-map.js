...
function showSingleLocatorMap(latitude, longitude, locator, bounds) {
    ...
    currentMarker = L.marker([latitude, longitude]).addTo(map)
        .bindPopup(translations.loc + locator)
        .openPopup();
...
function showTwoLocatorsMap(lat1, lon1, lat2, lon2, locator1, locator2, bounds1, bounds2, distance) {
    ...
    function updateDistanceDisplay() {
        ...
        if (distanceElement) {
            ...
            distanceElement.innerHTML = roundedDistance + ' km';
        }
    }
...
window.onload = function () {
    let urlLocator = getURLParameter("loc");
    if (urlLocator) {
        urlLocator = urlLocator.toUpperCase();
        let regex = /^[A-Z]{2}[0-9]{2}[A-Z]{2}$/;

        if (regex.test(urlLocator)) {
            let [latitude, longitude, bounds] = getCoordinatesFromLocator(urlLocator);
            showSingleLocatorMap(latitude, longitude, urlLocator, bounds);
        } else {
            alert(translations.invalidLocator);
        }
    }
