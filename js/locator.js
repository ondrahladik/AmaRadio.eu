...
function calculateVincentyDistance(lat1, lon1, lat2, lon2) {
    ...
    return (s / 1000).toFixed(2); // Distance in kilometers
}
...
function showTwoLocatorsMap(lat1, lon1, lat2, lon2, locator1, locator2, bounds1, bounds2, distance) {
    ...
    // Define custom icons with letters A and B
    ...
    // Update the distance display element
    ...
    // Define the bounds to include both locators
    ...
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(
            function (position) {
                ...
                // Convert coordinates to locator (use original coordinates without rounding)
                let locator = latLonToLocator(latitude, longitude);

                clearMap(); // Add this line to remove previous markers
                ...
                // Create popup content with rounded coordinates
                ...
                // Add new marker with locator as part of the popup
...
function updateLocatorFromGPS() {
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(function(position) {
            ...
            // Převést souřadnice na lokátor
            let gpsLocator = latLonToLocator(latitude, longitude);

            // Získat aktuální hodnoty textových polí
            ...
            // Vymazat locator2, pokud obsahuje hodnotu získanou z GPS
            if (currentLocator2 === gpsLocator) {
                console.log('Clearing locator2 because it matches GPS locator');
                locator2Input.value = '';
            }

            // Přidat novou hodnotu do textového pole ve formuláři 1
...
function updateLocatorToBFromGPS() {
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(function(position) {
            ...
            // Vymazat locator1, pokud obsahuje hodnotu získanou z GPS
            if (currentLocator1 === gpsLocator) {
                console.log('Clearing locator1 because it matches GPS locator');
                locator1Input.value = '';
            }

            // Přidat novou hodnotu do textového pole ve formuláři 2
...
let clickCount = 0;
let clickTimer;
const doubleClickThreshold = 300; // Časový práh pro dvojklik v milisekundách
...
function updateLocatorAFromMap(lat, lon) {
    ...
    // Nastavit hodnotu do textového pole pro Locator A
    locatorAInput.value = locator;
    isLocatorASet = true; // Označit, že Locator A je vyplněn
}
...
function updateLocatorBFromMap(lat, lon) {
    ...
    // Nastavit hodnotu do textového pole pro Locator B
    locatorBInput.value = locator;
    isLocatorBSet = true; // Označit, že Locator B je vyplněn
}
...
window.onload = function () {
    ...
    addLocationButton(); // Add the location button to the map
    ...
    map.on('mousemove', function (e) {
        ...
        if (locator) {
            ...
            mouseLocatorElement.style.display = 'flex'; // Zobrazit pozadí
        } else {
            mouseLocatorElement.style.display = 'none'; // Skrytí pozadí, pokud není lokátor
        }
    });

    map.on('mouseout', function () {
        let mouseLocatorElement = document.getElementById('mouse-locator');
        mouseLocatorElement.style.display = 'none'; // Skrytí pozadí při opuštění mapy
    });
    ...
    map.on('contextmenu', function(e) {
        ...
        if (clickCount === 1) {
            // Počkejte krátkou dobu, abyste zjistili, zda došlo k dalším kliknutím
        ...
        // Zabránit výchozímu chování pro kontextové menu
